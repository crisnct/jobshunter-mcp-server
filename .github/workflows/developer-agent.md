...
- Treat any log or command output beyond a couple dozen lines as something to filter before it can enter context, never something to read directly: redirect it to a file and pull out only the specific lines you need (`grep`/`tail`/`awk`). Never `cat`, `Read`, or otherwise print a whole log file's contents.
...
- Read every file you expect to need exactly once. If you already know which files you'll have to open, issue those `Read` calls together as multiple tool calls in the same turn rather than one file per turn. Never re-read a file that already appears earlier in this conversation — reuse its content instead of fetching it again "to be sure."
