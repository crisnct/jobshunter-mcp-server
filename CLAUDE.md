...
## AI agent workflows (gh-aw)

`.github/workflows/reviewer-agent.md` and `.github/workflows/developer-agent.md` define the two gh-aw ("GitHub
Agentic Workflows") agents that drive the `ai:*` label state machine — reviewing `ai:to_review` pull requests and
implementing `ai:ready` issues, respectively. Each has a compiled `*.lock.yml` counterpart, generated by
`gh aw compile` (`github/gh-aw` extension); `.github/workflows/compile-agentic-workflows.yml` runs that compiler on
every push touching either `.md` file and commits the regenerated lock file(s) automatically.

**Recompilation only matters for frontmatter changes.** Everything under the YAML frontmatter block (`on:`,
`permissions:`, `safe-outputs:` — including the `allowed`/`max` lists for `add-labels`/`remove-labels`, tool wiring,
etc.) is baked into the compiled job structure and requires `gh aw compile` to take effect. The markdown **body**
below the frontmatter (the actual instructions given to the model) is different: the compiler emits a
`{{#runtime-import <file>}}` directive for it rather than freezing the text, so the agent job re-reads
`reviewer-agent.md`/`developer-agent.md` live off the checked-out repository at run time. Editing the instructions
and pushing to `main` takes effect on the very next run — no recompilation step needed, and no need to wait for
`compile-agentic-workflows.yml`. Note that this is read from the **base branch** specifically: the agent job restores
`.github` from the base branch before running, so a pull request can never alter the reviewer's own instructions
from its own branch.
